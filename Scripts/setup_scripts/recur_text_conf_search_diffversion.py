...
def main():
    root_dir = './conf_text'
    projects = [p for p in os.listdir(root_dir) if p.endswith('-block_text')]

    data = {}

    for project in projects:
        directory = os.path.join(root_dir, project)
        files = [f for f in os.listdir(directory) if f.endswith('.json')]
        
        for file in files:
            with open(os.path.join(directory, file), 'r') as f:
                data[(project, file)] = json.load(f)

    results = []
    id_counter = 1

    for (project1, file1), content1 in data.items():
        for (project2, file2), content2 in data.items():
                        ...
                        for block1 in entity1["block_details"]:
                            for block2 in entity2["block_details"]:
                                ...
                                if similarity >= 0.5:
                                    ...
                                    if any(p >= 0.5 for p in proportions):
                                        result = {
                                            "id": id_counter,
                                            "merge_commitid": [entity1["merge_commitid"], entity2["merge_commitid"]],
                                            "recurBlockSrc": [project1, project2],
                                            "file": entity1["File"],
                                        ...
                                        result["recur_segment"] = common_segment
    ...
    fieldnames = ["id", "merge_commitid", "recurBlockSrc", "file", "Loc_details"]
    ...
    with open('./recur-block/text/diff-version/aospa-recur_textblock.csv', 'w', newline='') as csvfile:
